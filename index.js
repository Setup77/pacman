...
class Player {
    ...
    draw() {
        ...
        c.lineTo(this.position.x, this.position.y); // donne une bouche au joueur
        c.fillStyle = 'yellow';
        c.fill();
        c.closePath;
        c.restore();
...
function animate(){
  ...
  backgroundMusic.play();
    c.clearRect(0, 0, canvas.width, canvas.height); // elimine les traces carré du joueur lor du déplacement
     ...
     if(Math.hypot(powerUp.position.x - player.position.x, 
      powerUp.position.y - player.position.y) < powerUp.radius + player.radius ){
          ...
          ghosts.forEach(ghost =>{
             ghost.scared = true; 
             //---les ghosts changent de couleurs pendant 5 secs et le joueur est capable de les traversé
             setTimeout(() => {
                 ghost.scared = false;  
             }, 5000);
          });
...
//--- creéation et collisions des pellets
